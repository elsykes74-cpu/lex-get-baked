        ...
        <div className="max-w-[1440px] mx-auto lg:grid lg:grid-cols-[1fr_560px] lg:gap-20 lg:items-start">
          ...
          <div>
            ...
            <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} className="mb-6 mt-4">
              <p className="text-[9px] lg:text-[11px] tracking-[0.18em] uppercase font-semibold mb-2 text-muted">Dessert Lab</p>
              <h1 className="font-display text-[30px] lg:text-[56px] italic font-bold text-plum leading-tight">
            ...
            <motion.div
              ...
              className="glass-card rounded-[24px] p-5 mb-5 lg:hidden"
            >
            ...
            <div className="glass-card rounded-[18px] p-3.5 mb-5">
              ...
              <div className="flex justify-between">
                {STEPS.map((s, i) => (
                  <span
                    key={s}
                    className="text-[9px] lg:text-[11px] font-bold capitalize transition-colors"
                    style={{
                      color: i === step ? '#C9748F' : i < step ? '#2F2343' : 'rgba(47,35,67,0.28)',
                    }}
                  >
                    {i < step ? '✓' : s}
                  </span>
            ...
            <AnimatePresence mode="wait">
              {!isReview ? (
                ...
                >
                  ...
                  <div className={`grid grid-cols-2 gap-3 lg:gap-4 ${currentOptions?.length === 4 ? 'lg:grid-cols-4' : currentOptions?.length === 3 ? 'lg:grid-cols-3' : ''}`}>
                        ...
                        >
                          <div className="flex items-start justify-between mb-3">
                            <div
                              ...
                              style={{ background: `${opt.color}15`, border: `1.5px solid ${opt.color}28` }}
                            >
                              {opt.emoji}
                            </div>
                            {opt.tag && (
          ...
          <div className="hidden lg:block" style={{ position: 'sticky', top: '108px' }}>
            <div className="glass-card rounded-[28px] p-10">
              ...
              <div className="flex justify-center mb-10">
                ...
                >
                  ...
                  <div
                    ...
                    style={{
                      background: 'radial-gradient(ellipse, rgba(247,196,216,0.45) 0%, rgba(243,198,157,0.22) 50%, transparent 75%)',
                      filter: 'blur(24px)',
                      transform: 'scale(1.3)',
                    }}
                  />
                  <motion.div
                    ...
                    animate={{
                      scale: completedSteps > 0 ? [1, 1.05, 1] : 1,
                    }}
              ...
              <div className="space-y-2 mb-6">
                {(['Base', 'Filling', 'Finish', 'Size'] as StepKey[]).map(key => {
                  ...
                  return (
                    ...
                    >
                      <span className="text-[11px] font-bold text-muted uppercase tracking-wider">{key}</span>
                      <div className="flex items-center gap-2">
                        {chosen ? (
                          <>
                            <span className="text-base leading-none">{chosen.emoji}</span>
                            <span className="text-[14px] font-bold text-plum">{chosen.label}</span>
                          </>
                        ) : (
                          <span className="text-[13px] font-medium" style={{ color: 'rgba(47,35,67,0.28)' }}>—</span>
                        )}
                      </div>
                    </div>
                  );
              ...
              {/* Divider + price */}
              <div className="h-px mb-5" style={{ background: 'rgba(47,35,67,0.07)' }} />
              <div className="flex items-center justify-between mb-2">
                <div>
                  <p className="text-[11px] font-semibold text-muted uppercase tracking-wider mb-1">Estimated Price</p>
                  <div className="flex items-center gap-1.5">
                    <Sparkles size={12} strokeWidth={2} style={{ color: '#C9748F' }} />
                    <span className="text-[12px] font-semibold text-muted">Made fresh to order</span>
                  </div>
                </div>
                <AnimatePresence mode="wait">
                  <motion.p
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.38 }}>
      <div className="glass-card rounded-[24px] p-6 mb-4">
        ...
        <h2 className="font-display text-[20px] lg:text-[28px] font-bold text-plum mb-5">
